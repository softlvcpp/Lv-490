...
void XMLClient::WriteSystemInformation(string& xml_str) const noexcept
{
    XMLDocument xml_doc;
    //root
    XMLNode* root = xml_doc.NewElement(g_CLIENT_ROOT);
    ...
    //IP
    XMLElement* ip = xml_doc.NewElement(g_CLIENT_IPADRESS);//root->client->IPAddress
    ip->SetText(m_ip_address.c_str());
    client->InsertEndChild(ip);
    //MAC
    XMLElement* mac = xml_doc.NewElement(g_CLIENT_MACADRESS);//root->client->MacAddress
    mac->SetText(m_mac_address.c_str());
    client->InsertEndChild(mac);   

    //SystemInfo
    XMLElement* sys_info = xml_doc.NewElement(g_CLIENT_SYSTEMINFORMATION);//root->client->SystemInformation

    //HardDisk loop
    XMLElement* hard_disk;
    for (size_t i = 0; i < m_hard_disk_type_list.size(); ++i)
    {
        hard_disk = xml_doc.NewElement(g_CLIENT_HARDDISK);//root->client->SystemInformation->HardDisk
        ...
        XMLElement* pHardDisk_Used = xml_doc.NewElement(g_CLIENT_HARDDISK_USED);//root->client->SystemInformation->HardDisk
        pHardDisk_Used->SetText(m_hard_disk_used[i]);
        XMLElement* pHardDisk_Free = xml_doc.NewElement(g_CLIENT_HARDDISK_FREE);//root->client->SystemInformation->HardDisk
    ...
    //RAM
    XMLElement* ram = xml_doc.NewElement(g_CLIENT_TOTALRAM);//root->client->SystemInformation->TotalRAM
    ram->SetText(m_total_ram);
    sys_info->InsertEndChild(ram);
    //CPU
    XMLElement* cpu_branch = xml_doc.NewElement(g_CLIENT_CPU);//root->client->SystemInformation->CPU

    XMLElement* cpu_numbers = xml_doc.NewElement(g_CLIENT_CPU_NUMBERS);//root->client->SystemInformation->CPU
    cpu_numbers->SetText(m_cpu_numbers);
    cpu_branch->InsertEndChild(cpu_numbers);

    XMLElement* cpu_vendor = xml_doc.NewElement(g_CLIENT_CPU_VENDOR);//root->client->SystemInformation->CPU
    cpu_vendor->SetText(m_cpu_vendor.c_str());
    cpu_branch->InsertEndChild(cpu_vendor);

    XMLElement* cpu_speed = xml_doc.NewElement(g_CLIENT_CPU_SPEED);//root->client->SystemInformation->CPU
    cpu_speed->SetText(m_cpu_speed);
    cpu_branch->InsertEndChild(cpu_speed);
    sys_info->InsertEndChild(cpu_branch);

    XMLElement* process_info = xml_doc.NewElement(g_CLIENT_PROCESSINFORMATION);//root->client->SystemInformation->ProcessInformation

    XMLElement* process;

    for (auto it = m_processes.begin(); it != m_processes.end(); it++)//map iterator
    {
        process = xml_doc.NewElement(g_CLIENT_PROCESS);
        process->SetAttribute(g_CLIENT_PROCESS_ATTRIBUTE_NAME, it->second.c_str());
        process->SetAttribute(g_CLIENT_PROCESS_ATTRIBUTE_PID, it->first);
